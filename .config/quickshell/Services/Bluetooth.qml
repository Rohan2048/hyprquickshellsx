...
QtObject {
    ...
    // Safety net: liveMode should only ever be on while the popup's gear
    // view is actually visible. BluetoothPopup.qml forces liveMode = false
    // in its close() function, but if that path is ever bypassed (force
    // close, UI reload, crash-restart, etc.) this guarantees the scan
    // listener still can't run in the background indefinitely.
    property Timer liveModeSafety: Timer {
        interval: 120000  // 2 min hard cap
        running: root.liveMode
        onTriggered: root.liveMode = false
    }
    ...
    // liveMode is forced off when BluetoothPopup closes (see close() there)
    // and hard-capped by liveModeSafety above, so this can never scan
