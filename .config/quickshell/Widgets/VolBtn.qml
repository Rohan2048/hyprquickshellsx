...
Rectangle {
    ...
    scale: (hovered && !disabled) ? 1.02 : 1.0
    Behavior on scale {
        NumberAnimation { duration: Animations.hoverScaleDuration; easing.type: Animations.hoverScaleEasing }
    }

    implicitHeight: 30
    implicitWidth: root.icon !== "" ? implicitHeight : metrics.implicitWidth + 24

    // Hidden text used purely to measure the label's natural width, so
    // root.implicitWidth still behaves like it did with plain Text. The
    // visible label below is MarqueeText, which (being an Item) doesn't
    // report an implicit width of its own.
    Text {
        id: metrics
        visible: false
        text: root.text
